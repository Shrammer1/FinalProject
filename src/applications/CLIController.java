...
public class CLIController {
	...
	private void connect(String[] strings) {
		...
		try {
			ControllerAPI controllerIntf = (ControllerAPI) Naming.lookup("rmi://" + strings[1] + "/controller");
			appIntf = (AppAPI) controllerIntf.register(0);
			println("Connected ...");
		} catch (Exception e){
			e.printStackTrace();
		}
		...
		if(input[1].equals("switch")){
			if(input[2].equals("timeout")){
				try{
					...
					if(appIntf.setSwitchTimeout(input[3], newTimout)){
						println("Success");
					}else{
						println("Failure");
					}
